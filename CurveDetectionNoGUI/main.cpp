...
int main(int argc, char *argv[]) {
	...
	// find contours
	std::vector<Polygon> polygons = CurveDetector::findContours(image);
	...
	for (int i = 0; i < polygons.size(); i++) {
		...
		CurveDetector::detect(polygons[i].contour, 20000, 200, 0.03, 30, 90 / 180.0 * CV_PI, 80, 400, results);
		circles.insert(circles.end(), results.begin(), results.end());
	}

	if (circles.size() > 0) {
		std::cout << "Circle detected for " << argv[1] << std::endl;
	}
